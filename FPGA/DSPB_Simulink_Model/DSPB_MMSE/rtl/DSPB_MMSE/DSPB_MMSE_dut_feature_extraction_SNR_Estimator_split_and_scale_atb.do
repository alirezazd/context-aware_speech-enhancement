...
if {! [file exists $base_dir/DSPB_MMSE_dut_feature_extraction_SNR_Estimator_split_and_scale]} {
    file delete -force $base_dir/DSPB_MMSE_dut_feature_extraction_SNR_Estimator_split_and_scale
}

project new $base_dir DSPB_MMSE_dut_feature_extraction_SNR_Estimator_split_and_scale
...
if {[info exists quartus_dir] && [file isdirectory $quartus_dir]} {
    ...
    do "$base_dir/DSPB_MMSE/DSPB_MMSE_dut_feature_extraction_SNR_Estimator_split_and_scale_fpc.do"
} else {
...
vcom -quiet -O0 $base_dir/DSPB_MMSE/DSPB_MMSE_dut_feature_extraction_SNR_Estimator_split_and_scale_atb.vhd
vcom -quiet $base_dir/DSPB_MMSE/DSPB_MMSE_dut_feature_extraction_SNR_Estimator_split_and_scale_stm.vhd
...
if {$vcomfailed == 0} {
    onbreak {
        ...
        if {$regOK == 1} {
            ...
            if { [catch {exa mismatch_out_1_scaled_signal_amplitude_valid} mismatch] == 0 && $mismatch } {
                report_mismatch out_1_scaled_signal_amplitude_valid $cyclenum
            }
            if { [catch {exa mismatch_out_2_scaled_noise_amplitude_valid} mismatch] == 0 && $mismatch } {
                report_mismatch out_2_scaled_noise_amplitude_valid $cyclenum
            }
            if { [catch {exa mismatch_out_3_scaled_amplitude} mismatch] == 0 && $mismatch } {
                report_mismatch out_3_scaled_amplitude $cyclenum
            }
            if { [catch {exa mismatch_out_4_noise_count} mismatch] == 0 && $mismatch } {
                report_mismatch out_4_noise_count $cyclenum
            }
            if { [catch {exa mismatch_out_5_signal_count} mismatch] == 0 && $mismatch } {
                report_mismatch out_5_signal_count $cyclenum
            }
            if { [catch {exa mismatch_out_6_reset} mismatch] == 0 && $mismatch } {
                report_mismatch out_6_reset $cyclenum
            }
        } else {
    ...
    vsim -quiet -error 3473 -msgmode both -voptargs="+acc" -t ps DSPB_MMSE_dut_feature_extraction_SNR_Estimator_split_and_scale_atb 
    do $base_dir/DSPB_MMSE/DSPB_MMSE_dut_feature_extraction_SNR_Estimator_split_and_scale_atb.wav.do
