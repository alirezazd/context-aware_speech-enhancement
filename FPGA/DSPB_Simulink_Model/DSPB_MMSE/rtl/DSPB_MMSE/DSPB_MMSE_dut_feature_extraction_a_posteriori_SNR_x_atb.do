...
if {! [file exists $base_dir/DSPB_MMSE_dut_feature_extraction_a_posteriori_SNR_x]} {
    file delete -force $base_dir/DSPB_MMSE_dut_feature_extraction_a_posteriori_SNR_x
}

project new $base_dir DSPB_MMSE_dut_feature_extraction_a_posteriori_SNR_x
...
if {[info exists quartus_dir] && [file isdirectory $quartus_dir]} {
    ...
    do "$base_dir/DSPB_MMSE/DSPB_MMSE_dut_feature_extraction_a_posteriori_SNR_x_fpc.do"
} else {
...
vcom -quiet -O0 $base_dir/DSPB_MMSE/DSPB_MMSE_dut_feature_extraction_a_posteriori_SNR_x_atb.vhd
vcom -quiet $base_dir/DSPB_MMSE/DSPB_MMSE_dut_feature_extraction_a_posteriori_SNR_x_stm.vhd
...
if {$vcomfailed == 0} {
    onbreak {
        ...
        if {$regOK == 1} {
            ...
            if { [catch {exa mismatch_out_1_noise_power_snr_valid} mismatch] == 0 && $mismatch } {
                report_mismatch out_1_noise_power_snr_valid $cyclenum
            }
            if { [catch {exa mismatch_out_2_a_posteriori_snr} mismatch] == 0 && $mismatch } {
                report_mismatch out_2_a_posteriori_snr $cyclenum
            }
            if { [catch {exa mismatch_out_3_noise_power_spectrum} mismatch] == 0 && $mismatch } {
                report_mismatch out_3_noise_power_spectrum $cyclenum
            }
        } else {
    ...
    vsim -quiet -error 3473 -msgmode both -voptargs="+acc" -t ps DSPB_MMSE_dut_feature_extraction_a_posteriori_SNR_x_atb 
    do $base_dir/DSPB_MMSE/DSPB_MMSE_dut_feature_extraction_a_posteriori_SNR_x_atb.wav.do
