...
if {! [file exists $base_dir/DSPB_MMSE_dut_output_adapter_IFFT_Adapter_MagPhaseToComplex]} {
    file delete -force $base_dir/DSPB_MMSE_dut_output_adapter_IFFT_Adapter_MagPhaseToComplex
}

project new $base_dir DSPB_MMSE_dut_output_adapter_IFFT_Adapter_MagPhaseToComplex
...
if {[info exists quartus_dir] && [file isdirectory $quartus_dir]} {
    ...
    do "$base_dir/DSPB_MMSE/DSPB_MMSE_dut_output_adapter_IFFT_Adapter_MagPhaseToComplex_fpc.do"
} else {
...
vcom -quiet -O0 $base_dir/DSPB_MMSE/DSPB_MMSE_dut_output_adapter_IFFT_Adapter_MagPhaseToComplex_atb.vhd
vcom -quiet $base_dir/DSPB_MMSE/DSPB_MMSE_dut_output_adapter_IFFT_Adapter_MagPhaseToComplex_stm.vhd
...
if {$vcomfailed == 0} {
    onbreak {
        ...
        if {$regOK == 1} {
            ...
            if { [catch {exa mismatch_out_1_valid_x} mismatch] == 0 && $mismatch } {
                report_mismatch out_1_valid_x $cyclenum
            }
            if { [catch {exa mismatch_out_2_real_out} mismatch] == 0 && $mismatch } {
                report_mismatch out_2_real_out $cyclenum
            }
            if { [catch {exa mismatch_out_2_imag_out} mismatch] == 0 && $mismatch } {
                report_mismatch out_2_imag_out $cyclenum
            }
        } else {
    ...
    vsim -quiet -error 3473 -msgmode both -voptargs="+acc" -t ps DSPB_MMSE_dut_output_adapter_IFFT_Adapter_MagPhaseToComplex_atb 
    do $base_dir/DSPB_MMSE/DSPB_MMSE_dut_output_adapter_IFFT_Adapter_MagPhaseToComplex_atb.wav.do
