...
if {! [file exists $base_dir/DSPB_MMSE_dut_input_adapter_Hamming_window]} {
    file delete -force $base_dir/DSPB_MMSE_dut_input_adapter_Hamming_window
}

project new $base_dir DSPB_MMSE_dut_input_adapter_Hamming_window
...
if {[info exists quartus_dir] && [file isdirectory $quartus_dir]} {
    ...
    do "$base_dir/DSPB_MMSE/DSPB_MMSE_dut_input_adapter_Hamming_window_fpc.do"
} else {
...
vcom -quiet -O0 $base_dir/DSPB_MMSE/DSPB_MMSE_dut_input_adapter_Hamming_window_atb.vhd
vcom -quiet $base_dir/DSPB_MMSE/DSPB_MMSE_dut_input_adapter_Hamming_window_stm.vhd
...
if {$vcomfailed == 0} {
    onbreak {
        ...
        if {$regOK == 1} {
            ...
            if { [catch {exa mismatch_out_1_Input_Signal_Valid_x} mismatch] == 0 && $mismatch } {
                report_mismatch out_1_Input_Signal_Valid_x $cyclenum
            }
            if { [catch {exa mismatch_out_2_Input_Signal_x} mismatch] == 0 && $mismatch } {
                report_mismatch out_2_Input_Signal_x $cyclenum
            }
        } else {
    ...
    vsim -quiet -error 3473 -msgmode both -voptargs="+acc" -t ps DSPB_MMSE_dut_input_adapter_Hamming_window_atb 
    do $base_dir/DSPB_MMSE/DSPB_MMSE_dut_input_adapter_Hamming_window_atb.wav.do
