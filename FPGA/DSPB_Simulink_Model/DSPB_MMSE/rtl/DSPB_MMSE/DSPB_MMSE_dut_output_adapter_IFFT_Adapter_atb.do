...
if {! [file exists $base_dir/DSPB_MMSE_dut_output_adapter_IFFT_Adapter]} {
    file delete -force $base_dir/DSPB_MMSE_dut_output_adapter_IFFT_Adapter
}

project new $base_dir DSPB_MMSE_dut_output_adapter_IFFT_Adapter
...
if {[info exists quartus_dir] && [file isdirectory $quartus_dir]} {
    ...
    do "$base_dir/DSPB_MMSE/DSPB_MMSE_dut_output_adapter_IFFT_Adapter_fpc.do"
} else {
...
vcom -quiet -O0 $base_dir/DSPB_MMSE/DSPB_MMSE_dut_output_adapter_IFFT_Adapter_atb.vhd
vcom -quiet $base_dir/DSPB_MMSE/DSPB_MMSE_dut_output_adapter_IFFT_Adapter_stm.vhd
...
if {$vcomfailed == 0} {
    onbreak {
        ...
        if {$regOK == 1} {
            ...
            if { [catch {exa mismatch_out_1_valid} mismatch] == 0 && $mismatch } {
                report_mismatch out_1_valid $cyclenum
            }
            if { [catch {exa mismatch_out_2_imag_output} mismatch] == 0 && $mismatch } {
                report_mismatch out_2_imag_output $cyclenum
            }
            if { [catch {exa mismatch_out_2_real_output} mismatch] == 0 && $mismatch } {
                report_mismatch out_2_real_output $cyclenum
            }
        } else {
    ...
    vsim -quiet -error 3473 -msgmode both -voptargs="+acc" -t ps DSPB_MMSE_dut_output_adapter_IFFT_Adapter_atb 
    do $base_dir/DSPB_MMSE/DSPB_MMSE_dut_output_adapter_IFFT_Adapter_atb.wav.do
