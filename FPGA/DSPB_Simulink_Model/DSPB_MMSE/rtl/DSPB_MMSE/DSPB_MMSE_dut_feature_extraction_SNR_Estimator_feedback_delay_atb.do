...
if {! [file exists $base_dir/DSPB_MMSE_dut_feature_extraction_SNR_Estimator_feedback_delay]} {
    file delete -force $base_dir/DSPB_MMSE_dut_feature_extraction_SNR_Estimator_feedback_delay
}

project new $base_dir DSPB_MMSE_dut_feature_extraction_SNR_Estimator_feedback_delay
...
if {[info exists quartus_dir] && [file isdirectory $quartus_dir]} {
    ...
    do "$base_dir/DSPB_MMSE/DSPB_MMSE_dut_feature_extraction_SNR_Estimator_feedback_delay_fpc.do"
} else {
...
vcom -quiet -O0 $base_dir/DSPB_MMSE/DSPB_MMSE_dut_feature_extraction_SNR_Estimator_feedback_delay_atb.vhd
vcom -quiet $base_dir/DSPB_MMSE/DSPB_MMSE_dut_feature_extraction_SNR_Estimator_feedback_delay_stm.vhd
...
if {$vcomfailed == 0} {
    onbreak {
        ...
        if {$regOK == 1} {
            ...
            if { [catch {exa mismatch_out_2_valid_x} mismatch] == 0 && $mismatch } {
                report_mismatch out_2_valid_x $cyclenum
            }
            if { [catch {exa mismatch_out_1_output} mismatch] == 0 && $mismatch } {
                report_mismatch out_1_output $cyclenum
            }
        } else {
    ...
    vsim -quiet -error 3473 -msgmode both -voptargs="+acc" -t ps DSPB_MMSE_dut_feature_extraction_SNR_Estimator_feedback_delay_atb 
    do $base_dir/DSPB_MMSE/DSPB_MMSE_dut_feature_extraction_SNR_Estimator_feedback_delay_atb.wav.do
