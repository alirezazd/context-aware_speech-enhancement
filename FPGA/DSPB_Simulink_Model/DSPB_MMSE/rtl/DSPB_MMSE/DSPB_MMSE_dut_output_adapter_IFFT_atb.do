...
if {! [file exists $base_dir/DSPB_MMSE_dut_output_adapter_IFFT]} {
    file delete -force $base_dir/DSPB_MMSE_dut_output_adapter_IFFT
}

project new $base_dir DSPB_MMSE_dut_output_adapter_IFFT
...
if {[info exists quartus_dir] && [file isdirectory $quartus_dir]} {
    ...
    do "$base_dir/DSPB_MMSE/DSPB_MMSE_dut_output_adapter_IFFT_fpc.do"
} else {
...
vcom -quiet -O0 $base_dir/DSPB_MMSE/DSPB_MMSE_dut_output_adapter_IFFT_atb.vhd
vcom -quiet $base_dir/DSPB_MMSE/DSPB_MMSE_dut_output_adapter_IFFT_stm.vhd
...
if {$vcomfailed == 0} {
    onbreak {
        ...
        if {$regOK == 1} {
            ...
            if { [catch {exa mismatch_out_1_Valid1_x} mismatch] == 0 && $mismatch } {
                report_mismatch out_1_Valid1_x $cyclenum
            }
            if { [catch {exa mismatch_out_2_Output1} mismatch] == 0 && $mismatch } {
                report_mismatch out_2_Output1 $cyclenum
            }
        } else {
    ...
    vsim -quiet -error 3473 -msgmode both -voptargs="+acc" -t ps DSPB_MMSE_dut_output_adapter_IFFT_atb 
    do $base_dir/DSPB_MMSE/DSPB_MMSE_dut_output_adapter_IFFT_atb.wav.do
