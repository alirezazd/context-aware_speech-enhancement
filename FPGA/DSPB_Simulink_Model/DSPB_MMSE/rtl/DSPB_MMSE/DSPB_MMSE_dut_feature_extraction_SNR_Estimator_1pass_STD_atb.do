...
if {! [file exists $base_dir/DSPB_MMSE_dut_feature_extraction_SNR_Estimator_1pass_STD]} {
    file delete -force $base_dir/DSPB_MMSE_dut_feature_extraction_SNR_Estimator_1pass_STD
}

project new $base_dir DSPB_MMSE_dut_feature_extraction_SNR_Estimator_1pass_STD
...
if {[info exists quartus_dir] && [file isdirectory $quartus_dir]} {
    ...
    do "$base_dir/DSPB_MMSE/DSPB_MMSE_dut_feature_extraction_SNR_Estimator_1pass_STD_fpc.do"
} else {
...
vcom -quiet -O0 $base_dir/DSPB_MMSE/DSPB_MMSE_dut_feature_extraction_SNR_Estimator_1pass_STD_atb.vhd
vcom -quiet $base_dir/DSPB_MMSE/DSPB_MMSE_dut_feature_extraction_SNR_Estimator_1pass_STD_stm.vhd
...
if {$vcomfailed == 0} {
    onbreak {
        ...
        if {$regOK == 1} {
            ...
            if { [catch {exa mismatch_out_1_signal_STD_valid} mismatch] == 0 && $mismatch } {
                report_mismatch out_1_signal_STD_valid $cyclenum
            }
            if { [catch {exa mismatch_out_2_signal_STD} mismatch] == 0 && $mismatch } {
                report_mismatch out_2_signal_STD $cyclenum
            }
            if { [catch {exa mismatch_out_3_noise_STD_valid} mismatch] == 0 && $mismatch } {
                report_mismatch out_3_noise_STD_valid $cyclenum
            }
            if { [catch {exa mismatch_out_4_noise_STD} mismatch] == 0 && $mismatch } {
                report_mismatch out_4_noise_STD $cyclenum
            }
        } else {
    ...
    vsim -quiet -error 3473 -msgmode both -voptargs="+acc" -t ps DSPB_MMSE_dut_feature_extraction_SNR_Estimator_1pass_STD_atb 
    do $base_dir/DSPB_MMSE/DSPB_MMSE_dut_feature_extraction_SNR_Estimator_1pass_STD_atb.wav.do
