...
function [Enhanced_Magnitude_Spectrum] = Enhance_MMSE(A_Pos_SNR, A_Pri_SNR, Noisy_Magnitude_Spectrum)
    ...
    for i = 1 : length(Noisy_Magnitude_Spectrum)
        Enhanced_Magnitude_Spectrum(i) = ((sqrt(pi*V_k(i)))/(2*A_Pos_SNR(i))) * (((1 + V_k(i)) * SMBEssel0(V_k(i)/2)) + (V_k(i) * SMBEssel1(V_k(i)/2))) * Noisy_Magnitude_Spectrum(i);
    end
end
