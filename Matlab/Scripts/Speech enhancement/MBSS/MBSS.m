...
function [Noise_Power_Spectrum] = A_Posteriori_SNR(mag)
    ...
    for count = 1 : length(noise_mag)
        if((noise_mag(count) < (b * delta_tmp(count))) && (noise_mag(count) > (g * delta_tmp(count))))
...
function [Enhanced_Magnitude_Spectrum] = Enhance_MBSS(Noisy_Power_Spectrum, Noise_Power_Spectrum, Band_nu, SF)
    ...
    for count = 1:Band_nu
        ...
        for i = 1:Band_size
            if Enhanced_Power_Spectrum((count-1)*Band_size+i) <= 0.002 * Noisy_Power_Spectrum((count-1)*Band_size+i)
                Enhanced_Power_Spectrum((count-1)*Band_size+i) = 0.002 * Noisy_Power_Spectrum((count-1)*Band_size+i);
            end
        end
